...
class AppstoreSpider(CrawlSpider):
    ...
    def get_game_page(self,response):
        ...
        for game_page in cate_game_page[:1]:  #测试
    ...
    def cate_game(self,response):
        ...
        for cate_game in cate_game_url:
            game_url0= 'http://shouji.baidu.com/game/' + cate_game
            req = Request(game_url0,callback=self.game_get)
            yield req
    def game_get(self,reponse):
        sel = Selector(reponse)
        game= BaiduGameUrlItem()
        game['game_type_name']=sel.xpath("//div[@class='nav']//a/text()").extract()[-1]
        game['game_name']=sel.xpath("//div[@class='content-right']/h1/span/text()").extract()[0]
        game['game_id'] = sel.xpath("//span[@class='res-tag-ok']/text()").extract()[0]
        game['game_download_times'] = sel.xpath("//span[@class='download-num']/text()").extract()[0]
        game['game_size']=sel.xpath("//span[@class='size']/text()").extract()[0]
        # game['game_content']=sel.xpath("//div[@class='brief-long']/p/text()").extract()[-1]
        return game
